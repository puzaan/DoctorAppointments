...
const viewDoctorByNameAndTag=async(req,res)=>{
    ...
    console.log(regex);
    try {
        const done=await doctorSchema.find({fullName:{$regex:regex, '$options' : 'i'}});
        const done1=await doctorSchema.find({tag:{$regex:regex, '$options' : 'i'}});
        done.push(done1);
        if(done.length>0){
    ...
    doctorSchema.findOne({doctorId:req.params.id}).exec((error,done)=>{
        if(error){
            ...
            if(!done){
                ...
                doctorSchema.findOneAndUpdate({doctorId:req.params.id},{$push:{"tag":value}}).exec((error,done)=>{
                    if(done){
    ...
    doctorSchema.findOne({doctorId:req.params.id}).exec((error,done)=>{
        if(error){
            ...
            if(!done){
                ...
                doctorSchema.findOneAndUpdate({doctorId:req.params.id},{$pull:{"tag":value}}).exec((error,done)=>{
                    if(done){
    ...
    doctorSchema.findOne({doctorId:req.params.id}).exec((error,done)=>{
        if(error){
            ...
            if(!done){
                ...
                res.status(200).json({"msg":"Available Dates are as follow","data":done.available_dates,"doctorId":done.doctorId});
...
const viewAvailableTime=(req,res)=>{
    ...
    doctorSchema.findOne({doctorId:req.params.id}).exec(async(error,done)=>{
        if(error){
            res.status(500).json({"msg":"Server Error"});
            ...
            if(!done){
                ...
                res.status(200).json({"msg":"Available time data fetched",data:availableTime});
...
module.exports={viewAvailableTime,viewAvailableDates,viewDoctorByNameAndTag,addTagById,deleteTagById}