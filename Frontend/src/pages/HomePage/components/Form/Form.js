...
const Form = (props) => {
  ...
  const onSubmit = async (values) => {
    ...
    // for (let key in multipalFile) {
    //   formdata.append('multipalFile', multipalFile[key]);
    // }
    // formdata.ap
  ...
  useEffect(() => {
    const getTimelist = async () => {
      await fetch(`${url}/api/v1/public/doctor/view/timestamp/${formik.values.doctorId}?date=${fulldate}`)
  ...
  // };
  console.log(formik.values.doctorId);
          ...
          <Box component={Grid} marginBottom={{ xs: 10, sm: 0 }} container spacing={4}>
            ...
            <Grid item container xs={12} justifyContent={'center'} alignItems={'center'} flexDirection={'column'}>
              ...
              <Dialog open={open} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
                <DialogTitle id="alert-dialog-title">{'Your Appointment is booked'}</DialogTitle>
                <DialogContent>
