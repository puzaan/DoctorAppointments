...
import { UpdateAdmin, ViewAdmin } from '../apigetway/actions/AdminAction';
...
const AdminUpdate = () => {
  ...
  const updateName = () => {
    dispatch(UpdateAdmin(id, 'fullName', fullName));
  };

  const updateAddress = () => {
    dispatch(UpdateAdmin(id, 'address', address));
  };
  const updateGender = () => {
    dispatch(UpdateAdmin(id, 'gender', gender));
  };

  const updateEmail = () => {
    dispatch(UpdateAdmin(id, 'emailId', emailId));
  };
  const updateNumber = () => {
    dispatch(UpdateAdmin(id, 'contactNumber', contactNumber));
  };

  useEffect(() => {
    dispatch(ViewAdmin(id));
    if (success) {
