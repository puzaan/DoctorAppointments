...
import {
  AdminViewDoctor,
  AdminDoctorAddTime,
  AdminDoctordeleteTag,
  AdminDoctorAddTag,
  AdminDoctordeleteVideo,
  AdminDoctordeleteEducation,
  AdminDoctorAddEducation,
  AdminDoctorAddVideo,
  AdminDoctordeleteNo,
  AdminDoctorAddNo,
  AdminUpdateDoctor,
...
import { AdminPasswordChangeDoctor } from '../sections/auth/AdminDoctorPasswordChange';
import url from '../apigetway/mainUrl';

const AdminDoctorUpdate = () => {
  const { id } = useParams();
  ...
  const updateName = () => {
    dispatch(AdminUpdateDoctor(id, 'fullName', fullName));
  };

  const updateAddress = () => {
    dispatch(AdminUpdateDoctor(id, 'address', address));
  };
  const updateGender = () => {
    dispatch(AdminUpdateDoctor(id, 'gender', gender));
  };
  const updateNMCNumber = () => {
    dispatch(AdminUpdateDoctor(id, 'NMC_number', NMCnumber));
  };
  const updateEmail = () => {
    dispatch(AdminUpdateDoctor(id, 'emailId', emailId));
  };
  const updateFee = () => {
    dispatch(AdminUpdateDoctor(id, 'fee', fee));
  };
  const addNumber = () => {
    dispatch(AdminDoctorAddNo(id, contactNumber));
  };

  const deleteNumber = (value) => {
    dispatch(AdminDoctordeleteNo(id, value));
  };
  const addVideo = () => {
    dispatch(AdminDoctorAddVideo(id, videoList));
  };

  const addEducation = () => {
    dispatch(AdminDoctorAddEducation(id, education));
  };
  const deleteEducation = (value) => {
    dispatch(AdminDoctordeleteEducation(id, value));
  };
  const deleteVideo = (value) => {
    dispatch(AdminDoctordeleteVideo(id, value));
  };

  const addTag = () => {
    dispatch(AdminDoctorAddTag(id, tag));
  };

  const deleteTag = (value) => {
    dispatch(AdminDoctordeleteTag(id, value));
  };

  const addTime = () => {
    dispatch(AdminDoctorAddTime(id, date));
  };
  ...
  useEffect(() => {
    dispatch(AdminViewDoctor(id));
    const localToken = localStorage.getItem('adminInfo') ? JSON.parse(localStorage.getItem('adminInfo')) : null;
      ...
      {viewSuccess && (
        <>
          ...
          <Box display={'flex'} justifyContent={'center'}>
            <AdminPasswordChangeDoctor docId={id} />
          </Box>
        </>
      )}
    </>
