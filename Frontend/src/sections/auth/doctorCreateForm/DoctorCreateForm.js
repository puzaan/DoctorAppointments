...
// component
import { CreateDoctor } from '../../../apigetway/actions/DoctorAction';
...
export default function DoctorCreateForm() {
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    fee: Yup.number().min(1, 'Fee cannot bo less than Zero').required('Please Enter price'),
    contactNumber: Yup.string().matches(phoneRegExp, 'Phone number is not valid').required('Enter your contact Number'),
    NMC_number: Yup.string().matches(phoneRegExp, 'NMC number is not valid').required('Enter your NMC Number'),
    speciality: Yup.string().min(2, 'Too Short!').max(50, 'Too Long!').required('Enter your name'),
  });
  ...
  const formik = useFormik({
    initialValues: {
      ...
      gender: '',
      address: '',
      age: new Date().toISOString().replace('-', '/').split('T')[0].replace('-', '/'),
      speciality: '',
      NMC_number: '',
      fee: '',
    },
  ...
  useEffect(() => {
    if (success) {
      ...
      navigate('/superadmin/doctor', { replace: true });
    }
    console.log(success);
